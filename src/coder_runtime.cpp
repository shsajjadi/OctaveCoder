...
namespace coder
{
  ...
  {
    output.append (octave_value(evaluate(nargout,endkey,short_circuit),true));
  }
  Expression::operator bool ()
  {
    bool expr_value = false;
    octave_value retval (evaluate ( 1, Endindex(), true),true);
    ...
    for (int k = 0; k < len; k++)
      {
        ...
        auto& elt = *p++;
        octave_value tmp (elt->evaluate(1, endkey, false), true);
        if (tmp.is_defined ())
          {
            if (tmp.is_cs_list ())
              {
  ...
  get_struct_index (const Ptr_list& arg, const Endindex& endkey)
  {
    ...
    return octave_value (val.val, true);
  }
  coder_value
  ...
  Index::evaluate_n(coder_value_list& output, int nargout, const Endindex& endkey, bool short_circuit)
  {
    ...
    if (base_expr_val.is_undefined ())
      base_expr_val = octave_value(expr->evaluate ( 1, endkey), true);
  ...
  Index::lvalue (coder_value_list& in_idx)
  {
    ...
    octave_value base_obj = octave_value(retval.value (arg1), true);
  ...
  {
    octave_value left ( a->evaluate(nargout,endkey,short_circuit), true);
    octave_value right ( b->evaluate(nargout,endkey,short_circuit), true);
  ...
  {
    octave_value val ( a->evaluate(nargout,endkey,short_circuit), true);
    if (val.get_count () == 1)
    ...
    try
      {
        ...
        octave_value rhs_val ( rhs->evaluate (1,endkey), true);
  ...
  {
    octave_value val;
    octave_value a ( op_lhs->evaluate(1,endkey), true);
    if (a.is_defined ())
      {
        octave_value b ( op_rhs->evaluate (1,endkey), true);
        if (b.is_defined ())
          {
            octave::type_info& ti = octave::interpreter::the_interpreter () ->get_type_info ();
  ...
  {
    octave_value ov_base ( base->evaluate( 1,endkey,short_circuit), true);
    octave_value ov_increment ( inc.is_valid() ?
      octave_value(inc->evaluate(1,endkey,short_circuit), true) :
      octave_value(1.0));
    octave_value ov_limit ( limit->evaluate(1,endkey,short_circuit), true);
  ...
  And::evaluate( int nargout, const Endindex& endkey, bool short_circuit)
  {
    if (short_circuit)
      {
        octave_value left ( a->evaluate( 1,endkey,short_circuit), true);
        if (left.ndims () == 2 && left.rows () == 1 && left.columns () == 1)
          {
            ...
            octave_value right ( b->evaluate( 1,endkey,short_circuit), true);
  ...
  eval_dot_separated_names (Index& index_expr)
  {
    ...
    Endindex endindex;
    coder_value_list idx;
    octave_value base_expr_val = octave_value(expr->evaluate ( 1, endindex), true);
  ...
  Handle::evaluate( int nargout, const Endindex& endkey, bool short_circuit)
  {
    ...
    octave_value partial_expr_val = octave_value(expr->evaluate (1, endindex), true);
    ...
    for (auto& row : mat)
      {
        ...
        for (auto& col : row)
          {
            octave_value tmp ( col->evaluate(1,endkey), true);
            if (tmp.is_undefined ())
              {
    ...
    for (auto& row : cel)
      {
        ...
        for (auto& col : row)
          {
            octave_value tmp ( col->evaluate(1), true);
            if (tmp.is_undefined ())
              {
  ...
  {
    octave_value label_value ( label->evaluate (1), true);
    ...
    if (elt.size()==2)
      {
        ...
        octave_value init_val ( expr->evaluate (1), true);
  ...
  {
    output.append (octave_value (expr->evaluate (nargout), true));
  }
  void
